	...
	class WC_AM_Client_2_7K2 {
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '' ) {
			...
			if ( is_admin() ) {
				...
				$this->wc_am_settings_menu_title         = $this->software_title . esc_html__( ' Activation', 'eae' );
				$this->wc_am_settings_title              = $this->software_title . esc_html__( ' Activation', 'eae' );
				$this->wc_am_menu_tab_activation_title   = esc_html__( 'API Key Activation', 'eae' );
				$this->wc_am_menu_tab_deactivation_title = esc_html__( 'API Key Deactivation', 'eae' );
...
?>
		...
		public function check_external_blocking() {
			...
			if ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
				...
				if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
					?>
					<div class="notice notice-error">
						<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %1$s updates. Please add %2$s to %3$s.', 'eae' ), $this->software_title, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>' ); ?></p>
					</div>
					<?php
				}
			}
		...
		public function config_page() {
			...
			?>
			<div class='wrap'>
				<h2><?php esc_html_e( $this->wc_am_settings_title, 'eae' ); ?></h2>
		...
		public function load_settings() {
			...
			add_settings_section(
				...
				esc_html__( 'API Key Activation', 'eae' ),
				array(
					$this,
					'wc_am_api_key_text',
				),
			...
			add_settings_section(
				'deactivate_button',
				esc_html__( 'API Deactivation', 'eae' ),
				array(
					$this,
			...
			add_settings_field(
				'deactivate_button',
				esc_html__( 'Deactivate API Key', 'eae' ),
				array(
					$this,
		...
		public function wc_am_api_key_status() {
			if ( $this->get_api_key_status( true ) ) {
				...
				$license_status_check = esc_html__( 'Deactivated', 'eae' );
			}
			...
			if ( ! empty( $_REQUEST['option_page'] ) && $_REQUEST['option_page'] != $this->wc_am_deactivate_checkbox_key ) {
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( $activate_results['success'] === true && $activate_results['activated'] === true ) {
						add_settings_error( 'activate_text', 'activate_msg', sprintf( __( '%s activated. ', 'eae' ), esc_attr( $this->software_title ) ) . esc_attr( "{$activate_results['message']}." ), 'updated' );
						update_option( $this->wc_am_activated_key, 'Activated' );
						update_option( $this->wc_am_deactivate_checkbox_key, 'off' );
					}

					if ( $activate_results == false && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
						add_settings_error( 'api_key_check_text', 'api_key_check_error', esc_html__( 'Connection failed to the License Key API server. Try again later. There may be a problem on your server preventing outgoing requests, or the store is blocking your request to activate the plugin/theme.', 'eae' ), 'error' );
			...
			if ( $options == 'on' && $activation_status == 'Activated' && $this->data[ $this->wc_am_api_key_key ] != '' ) {
				...
				if ( $activate_results['success'] === true && $activate_results['deactivated'] === true ) {
					if ( ! empty( $this->wc_am_activated_key ) ) {
						update_option( $this->wc_am_activated_key, 'Deactivated' );
						add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', esc_html__( 'API Key deactivated. ', 'eae' ) . esc_attr( "{$activate_results['activations_remaining']}." ), 'updated' );
					}
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?>
			<span class="description"><?php esc_html_e( 'Deactivates an API Key so it can be used on another blog.', 'eae' ); ?></span>
			<?php
		}
